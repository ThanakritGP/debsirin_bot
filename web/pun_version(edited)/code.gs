...
const SPREADSHEET_ID = "1JsUATF3MxunvbQKdwqn_GjorbFx_yU8G-f5zDA8Ncn8"; 
  ...
  try {
    ...
    if (verifySheet) {
      ...
      for (let i = 1; i < verifyData.length; i++) {
        // คอลัมน์ที่ 2 (Index 1) คือ Student ID ในชีต verify
    ...
    for (const currentSheet of allSheets) {
      ...
      if (currentSheetName === VERIFY_SHEET_NAME || currentSheetName === ADMIN_USERS_SHEET || currentSheetName.toUpperCase().includes(TEMPLATE_SHEET_IDENTIFIER)) continue; 
...
// ===================================================
// ADMIN FUNCTIONS (BACKEND) - ต้องมี Token ตรวจสอบ
// ===================================================

/**
 * ฟังก์ชันสำหรับโหลดเนื้อหาของหน้า Admin Dashboard (Content)
 * (ไม่ได้ตรวจสอบ Token ในฟังก์ชันนี้ แต่ควรตรวจสอบก่อนแสดงผลในฝั่ง Client)
 */
...
/**
 * ดึงตัวชี้วัดสถิติรวมของ Admin Dashboard
 * ต้องใช้ token ในการเข้าถึง
 */
function getAdminMetrics(token) {
    const adminCheck = checkAdminToken(token);
    if (!adminCheck.success) {
        return { error: adminCheck.message };
    }
    ...
    try {
        ...
        for (const studentSheet of allSheets) {
            ...
            // ข้ามชีตที่ไม่ต้องการนับ (verify, admin_users หรือ template)
            if (sheetName === VERIFY_SHEET_NAME || sheetName === ADMIN_USERS_SHEET || sheetName.toUpperCase().includes(TEMPLATE_SHEET_IDENTIFIER)) {
                continue;
            }
...
/**
 * 1. ตรวจสอบสิทธิ์ผู้ดูแลระบบจากชีต admin_users และสร้าง Token
 */
...
/**
 * สร้างหน้า Admin Page โดยดึงรายชื่อชีตย่อยที่เก็บข้อมูลนักเรียน
 * ต้องใช้ token ในการเข้าถึง
 */
function getAdminPage(token) {
    const adminCheck = checkAdminToken(token);
    if (!adminCheck.success) {
        // หากไม่ผ่านการตรวจสอบ ควรส่งหน้า Login หรือ Error กลับไป (ในที่นี้ส่ง Error message)
        return { error: adminCheck.message }; 
    }
    ...
    // กรองเอาเฉพาะชื่อชีตที่เป็นข้อมูลนักเรียน
    const studentSheetNames = allSheets
        .map(sheet => sheet.getName())
        .filter(name => 
            name !== VERIFY_SHEET_NAME && 
            name !== ADMIN_USERS_SHEET &&
            !name.toUpperCase().includes(TEMPLATE_SHEET_IDENTIFIER)
        );
    ...
    // ต้องตรวจสอบในฝั่ง Client (JS) อีกครั้งว่า Token ยังถูกต้องก่อนแสดงผล
    return template.evaluate().getContent();
}

/**
 * ตรวจสอบ Token ของ Admin ที่ส่งมาจาก Client
 */
...
// ===================================================
// NEW ADMIN FUNCTION: SEARCH STUDENT DATA (FOR ADMIN)
// ===================================================

/**
 * ค้นหาข้อมูลนักเรียนโดยใช้เลขประจำตัว (สำหรับ Admin Dashboard)
 */
function searchStudentData(token, studentIdFromClient) {
    const adminCheck = checkAdminToken(token);
    if (!adminCheck.success) {
        return { error: adminCheck.message };
    }
    ...
    // Index ในชีตย่อย (A, B, C, D, E)
    const DATA_CLASS_LEVEL = 0; 
    const DATA_ROOM = 1;         
    const DATA_STUDENT_NO = 2;
    const DATA_ID = 3;             
    const DATA_FULL_NAME = 4;
    
    const HEADERS = ["ชั้น", "ห้อง", "เลขที่", "เลขประจำตัว", "ชื่อ-นามสกุล"];

    try {
        const ss = SpreadsheetApp.openById(spreadsheetId);
        
        // 1. ตรวจสอบสถานะการยืนยันตัวตนก่อน
        ...
        for (const currentSheet of allSheets) {
            ...
            for (let j = 0; j < sheetData.length; j++) {
                const row = sheetData[j];
                if (row[DATA_ID] && String(row[DATA_ID]).trim() === studentId) {
                    studentFound = true;
                    studentDataRow = [
                        (row[DATA_CLASS_LEVEL] || "").toString().trim(), 
                        (row[DATA_ROOM] || "").toString().trim(),          
                        (row[DATA_STUDENT_NO] || "").toString().trim(),
                        String(row[DATA_ID]).trim(),                                                    
                        (row[DATA_FULL_NAME] || "").toString().trim()
                    ];
                    break; 
                }
            }
        ...
        // 3. คืนค่าผลลัพธ์
        if (studentFound) {
            const tableHtml = createSearchDataTable([studentDataRow], HEADERS, isVerified);
            return { 
                status: "success", 
                html: tableHtml
            };
            ...
            return { 
                status: "not_found", 
                html: '<div class="alert alert-danger text-center">ไม่พบข้อมูลนักเรียนด้วยเลขประจำตัวนี้</div>'
            };
        }

    } catch (e) {
        Logger.log("Error in searchStudentData: " + e.message + " Stack: " + e.stack);
        return { status: "error", error: "เกิดข้อผิดพลาดในการค้นหาข้อมูล: " + e.message };
    }
}
 ...
 * ต้องใช้ token ในการเข้าถึง
 */
function getAllStudentData(token) {
    const adminCheck = checkAdminToken(token);
    if (!adminCheck.success) {
        return { error: adminCheck.message };
    }
    ...
    try {
        ...
        // 1. ดึง ID นักเรียนที่ยืนยันแล้ว
        const verifySheet = ss.getSheetByName(VERIFY_SHEET_NAME);
        if (verifySheet && verifySheet.getLastRow() > 1) {
            // ดึงเฉพาะคอลัมน์ B (ID) เริ่มจากแถว 2 (สมมติว่าแถว 1 คือหัวตาราง)
            const verifyData = verifySheet.getRange(2, 2, verifySheet.getLastRow() - 1, 1).getValues(); 
        ...
        for (const currentSheet of allSheets) {
            ...
            // *** ใช้ getRange() เพื่ออ่านข้อมูล เริ่มจากแถวที่ 6 (DATA_START_ROW) คอลัมน์ที่ 1 (A) ***
            const numRows = lastRow - DATA_START_ROW + 1;
            // ต้องตรวจสอบว่า lastColumn มีค่ามากกว่า 0 ก่อน
            ...
            for (let j = 0; j < sheetData.length; j++) { 
                ...
                // ตรวจสอบว่าคอลัมน์ Student ID มีข้อมูลจริง
                if (studentId) {
